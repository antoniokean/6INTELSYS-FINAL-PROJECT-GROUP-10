...
def resolve_results_paths(output_dir, preset_name, ablation_name):
    preset_dir = PRESET_DIR_NAMES[preset_name]
    config_path = output_dir / "configs" / preset_dir
    history_path = output_dir / "logs" / preset_dir
    results_path = output_dir / "results" / preset_dir
    if ablation_name != "none":
        config_path = config_path / "ablations" / ablation_name
        history_path = history_path / "ablations" / ablation_name
        results_path = results_path / "ablations" / ablation_name

    return {
        "config_dir": config_path,
        "log_dir": history_path,
...
def main():
    ...
    resolved_paths = resolve_results_paths(args.output_dir, args.preset, args.ablation)
    summary_path = resolved_paths["results_dir"] / "summary.json"
    history_path = resolved_paths["log_dir"] / "history.json"
    config_path = resolved_paths["config_dir"] / "run_config.json"
