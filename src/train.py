...
def resolve_preset_output_dirs(base_output_dir, preset_name):
    preset_dir_name = PRESET_OUTPUT_DIR_NAMES[preset_name]
    return {
        "configs": base_output_dir / "configs" / preset_dir_name,
        "logs": base_output_dir / "logs" / preset_dir_name,
        "results": base_output_dir / "results" / preset_dir_name,
    }
...
def validate_tensor(name, tensor, phase_name, batch_index, sample_paths):
    ...
    sample_preview = ", ".join(sample_paths[:5])
...
def run_epoch(model, loader, criterion, optimizer, scaler, device, phase_name, log_every):
    ...
    for batch_index, (features, labels, sample_paths) in enumerate(loader, start=1):
        features = features.to(device, non_blocking=True)
        if device.type == "cuda":
            features = features.contiguous(memory_format=torch.channels_last)
        labels = labels.to(device, non_blocking=True)
        validate_tensor("input features", features, phase_name, batch_index, list(sample_paths))
        ...
        with torch.set_grad_enabled(is_training):
            with autocast_context():
                ...
                validate_tensor("logits", logits, phase_name, batch_index, list(sample_paths))
                loss = criterion(logits, labels)
                validate_tensor("loss", loss, phase_name, batch_index, list(sample_paths))
...
def evaluate_loader_with_outputs(model, loader, criterion, device, phase_name, log_every):
    model.eval()
    ...
    with torch.no_grad():
        for batch_index, (features, labels, sample_paths) in enumerate(loader, start=1):
            ...
            validate_tensor("probabilities", probabilities, phase_name, batch_index, list(sample_paths))
...
def open_generated_images(curves_path, spectrogram_path):
    opened_any = False

    for path in (curves_path, spectrogram_path):
...
def maybe_run_eval_after_training(args, output_dirs):
    ...
    command = [sys.executable, "src/eval.py", "--preset", args.preset]
...
def save_outputs(output_dirs, model, label_map, history, summary, threshold_tuning, args, preview_rows):
...
def main():
    ...
    seed_was_randomized = args.randomize_run or args.seed is None
    if seed_was_randomized:
        args.seed = random.SystemRandom().randrange(0, (2**32) - 1)
    set_seed(args.seed)
    device = resolve_device(args.device)
    output_dirs = resolve_preset_output_dirs(args.output_dir, args.preset)
    ...
    if len(split_to_rows["testing"]) > 0:
        ...
        test_eval = {"loss": None, "accuracy": None, "probabilities": np.empty((0, len(label_map))), "labels": np.empty((0,), dtype=np.int64)}
    ...
    image_paths = save_outputs(output_dirs, model, label_map, history, summary, threshold_tuning, args, preview_rows)
    print(
    ...
    print(f"Training complete. Outputs saved under: {args.output_dir}")
    maybe_run_eval_after_training(args, output_dirs)
    if not args.no_open_files:
        open_generated_images(image_paths["training_curves"], image_paths["sample_log_mel"])
